 ...
 * @brief Check if counter deltas match between a parent and its member aggregate.
 * @param ifname            interface name
 * @param other_ifname      other interface name
 * @param sock              socket
 * @param member_count      expected member observations per broadcast packet; one means exact equality
 * @param monitored_msgs    array of monitored message types
 * @param monitored_msg_cnt number of monitored message types
                                          ...
                                          size_t member_count, const int *monitored_msgs, size_t monitored_msg_cnt)
{
    ...
    for (size_t i = 0; i < monitored_msg_cnt; i++) {
        ...
        uint64_t additional_member_observations = other_delta - delta;
        if (additional_member_observations % (member_count - 1) != 0 ||
            additional_member_observations / (member_count - 1) > delta) {
            return false;
        }
    }
    return true;
                                                ...
                                                size_t monitored_msg_cnt)
{
    ...
    // Each IPv4 VLAN TX packet is observed on one member for unicast or all direct members for broadcast.
